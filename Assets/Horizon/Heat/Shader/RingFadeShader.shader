Shader "Horizon/Planet Ring (Distance Fade)" {
	Properties {
		...
		_Distance("Distance Fade", Range(0, 1000)) = 10
		_MinDistance("Minimum Draw Distance", Range(0, 1000)) = 10
	}
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float distMult = length(_WorldSpaceCameraPos - IN.worldPos) * (1 / _Distance) - (_MinDistance / _Distance);
			if (distMult < 0) distMult = 0;
			else if (distMult > 1) distMult = 1;

			float2 p = float2(distance, 0);
			fixed4 c = tex2D (_MainTex, p) * _Color;
			o.Albedo = c.rgb;
			o.Alpha = (c.r + c.g + c.b) / 3 * (distance > 0 && distance < 1 ? 1 : 0) * distMult;
			...
			o.Metallic = _Metallic * c.a * bump * distMult;
			o.Smoothness = _Glossiness * c.a * bump * distMult;
			o.Normal = bump * (tex2D(_NormalTex, xy * 10) / 10 + float3(0,0.9,0));
			clip(distance > 0 && distance < 1 && distMult > 0 ? 1 : -1);
			o.Occlusion = 1;
